...
import com.japantv.settop.DetailDialogActivity;
    ...
    private void setCoverflowData(){
		...
		coverFlow.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if(position != -1){
					 mCustomPopup2 = new Detail2DialogActivity(RestoreActivity.this, xPop2PlayClickListener, xPop2PlayKeyListener, xPop2RestoreClickListener, xPop2RestoreKeyListener, "delete");
			...
			// TODO Auto-generated method stub
			 if(keyCode == KeyEvent.KEYCODE_DPAD_CENTER || keyCode == KeyEvent.KEYCODE_ENTER) {
					if(broadcastList.get((int)mPosition) == null)
		 				return true;
					if(!broadcastList.get((int)mPosition).getType().equals("")){
						type = "etc";
						Log.e(null, "click2");
						/*if(!isClicked){
							isClicked = true;
	...
	private boolean getImages(){
		...
		try{
			...
			for(int i = 0; i < jArray.length(); i++){
				...
				tempList.setType(json_data.getString("vod_code"));
...
//				mImages = new ImageView[mImageIds.length];
	...
	class WishTask extends AsyncTask<String, Integer, Long> {
    	...
    	protected void onPostExecute(Long result) {
    		...
    		dismissProgress();
    			AlertDialog.Builder alt_bld = new AlertDialog.Builder(RestoreActivity.this);
			    alt_bld.setMessage("ライブラリから削除されました。");
			    alt_bld.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
		            public void onClick(DialogInterface dialog, int which) {
		            	 clickOkButton();
		                dialog.dismiss();
